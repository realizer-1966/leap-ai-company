"""
인사이틱스(Insytics) MCP 도구 서버 샘플.

LeapChat의 HTTP API 도구로 연결할 수 있는 MCP 서버.
Streamable HTTP 전송을 사용해 외부에서 HTTP로 호출 가능.

실행:
    source /root/workspace/mcp-demo-venv/bin/activate
...
# MCP 서버 생성 (streamable HTTP 전송)
# 0.0.0.0 바인딩 → Android 앱(LeapChat)이 접근 가능
mcp = FastMCP(
    "insytics-mcp",
    host="0.0.0.0",
    port=9000,
...
if __name__ == "__main__":
    ...
    print("tools: get_current_time, get_today_date, compute_sum, random_number, count_words, unit_convert, http_get")
